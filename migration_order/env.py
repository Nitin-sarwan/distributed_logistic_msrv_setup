"""Alembic environment for order_db.

The third separate environment: `migration/` owns `user_db`, `migration_partner/`
owns `partner_db`, and this one owns `order_db`. They share nothing but the
...
Run it with the matching config file, from the repo root:
    ...
    alembic -c alembic_order.ini upgrade head

Forgetting `-c` runs the *user* migrations instead. They will report "nothing to
do" against order_db and then stamp order_db's version table with user revision
ids, which is a genuinely annoying mess to unpick.
"""
...
from src.services.orderServices.models import (  # noqa: F401
    order_model,
    outbox_model,
    processed_event_model,
    status_history_model,
)
