...
def check_database_connection(retries: int = 5, delay: float = 2) -> None:
    for attempt in range(1, retries + 1):
        try:
            ...
            logger.info("Database connected successfully (%s)", settings.order_db_name)
            return
...
# No create_tables() here. order_db is Alembic's from its first revision — see
# MIGRATIONS.md §8 rule 6 for what create_all() does to a migration history.
